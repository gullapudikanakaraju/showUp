module.exports= function(request, response){
	...
	data.posted_by= request.session.user_name;
			...
			groups_model.findOne({city: result.city, locality: result.locality}, function(e1, r1){
				...
				else
				{
					...
					if(r1 != null)
					{
						...
						async.parallel({
							update_dealers_collection : function(callback)
							{
								...
								dealer_model.update({email : request.session.email }, { $push : {groups : r1.group_name}}, callback);
							},
							...
							else
							{
								...
								response.json({
									success: true,
									message: 'prop creation successfull and added to room successfully'
								});
							}
						});
					}
					else
					{
						...
						y.members= members;
						y.created_by= request.session.user_name;
						console.log('y is ', y);
						...
						async.parallel({
							...
							update_dealers_collection: function(callback){
								var dealer_model = require('../schemas/dealer_schema.js');
								dealer_model.update({user_name : request.session.user_name }, { $push : {groups : y.group_name}}, callback);
							}
						}, function(e3, r3){
							if(e3)
							{
